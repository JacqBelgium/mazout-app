...
if short_term:
    ...
    with col1:
        st.metric(
            label="Current FOD Official Price",
            value=f"€ {official_price:.4f} / L"
        )
        
    with col2:
        st.metric(
            label="Estimated Official Price",
            value=f"€ {short_term['predicted_official_liter']:.4f} / L",
            delta=f"{short_term['delta_per_liter']:.4f} / L"
        )
        # Punt 2: Compact direct onder de tweede kolom geplaatst
        impact_val = short_term.get('impact_2000l', short_term['delta_per_liter'] * 2000)
        st.markdown(f"<div class='compact-impact'>Impact >2000 liters: € {impact_val:.2f}</div>", unsafe_allow_html=True)
